...
describe('DomesticScreen - 入力完了処理', () => {
  ...
  describe('バグ修正: input.context.commandsがCommandPanelに渡される', () => {
    it('BATTLEコマンドを含む全コマンドがCommandPanelに渡される', () => {
      ...
      const battleCommand: Command = createMockCommand({
...
describe('DomesticScreen - ユニット選択モード', () => {
  ...
  const mockSelfUnitCommand: Command = createMockCommand({
    commandId: 'cmd_buff_unit',
    name: '味方強化',
    targetType: CommandTargetType.SELF_UNIT,
  });

  const mockEnemyUnitCommand: Command = createMockCommand({
  ...
  const mockEnemyNationCommand: Command = createMockCommand({
  ...
  const mockSelfNationCommand: Command = createMockCommand({
    commandId: 'cmd_train',
    name: '訓練',
  });
